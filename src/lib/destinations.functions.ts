...
import { staticCategories, staticDestinations } from "@/lib/static-data";

// NOTE: temporarily serving static data instead of querying Supabase, so the
// site works on deployments (e.g. a fresh Vercel project) that don't have
// SUPABASE_URL / SUPABASE_PUBLISHABLE_KEY configured yet. To go back to
// live Supabase data, restore the createClient-based implementation below
// (kept commented out) and remove the static-data import above.
//
...
export const listCategories = createServerFn({ method: "GET" }).handler(async () => {
  ...
  // Previous Supabase-backed implementation:
...
export const listDestinations = createServerFn({ method: "GET" }).handler(async () => {
  return [...staticDestinations].sort((a, b) => a.name.localeCompare(b.name));
  // Previous Supabase-backed implementation:
  // const supabase = serverSupabase();
  // const { data, error } = await supabase
  //   .from("destinations")
